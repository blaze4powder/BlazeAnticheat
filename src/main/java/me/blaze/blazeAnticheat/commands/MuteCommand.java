...
public class MuteCommand implements CommandExecutor {
    ...
    public MuteCommand(BlazeAnticheat plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!plugin.getConfig().getBoolean("commands.mute.enabled")) return true;
        
        String p = plugin.getConfig().getString("permissions.mute");
        ...
        if (args.length == 0) {
            sender.sendMessage(msg("messages.usage-mute"));
            return true;
        }
