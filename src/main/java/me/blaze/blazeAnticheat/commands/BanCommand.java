...
public class BanCommand implements CommandExecutor {
    ...
    public BanCommand(BlazeAnticheat plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!plugin.getConfig().getBoolean("commands.ban.enabled")) return true;
        ...
        if (args.length == 0) {
            sender.sendMessage(msg("messages.usage-ban"));
            return true;
        }
        ...
        plugin.getBanManager().ban(t.getUniqueId(), r, sender.getName(), ms);
        
        sender.sendMessage(msg("messages.ban-success")
